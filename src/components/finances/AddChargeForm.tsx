...
const AddChargeForm = ({ isOpen, onClose, farmer }: AddChargeFormProps) => {
  ...
  // Find "Other income" ledger (matching name "Other income")
  const otherIncomeLedger = useMemo(() => {
    return ledgers.find((ledger) =>
      ledger.name.toLowerCase() === 'other income'
    );
  }, [ledgers]);
  ...
  const handleSubmit = () => {
    if (!formData.amount || !otherIncomeLedger || !farmerLedger) {
      if (!otherIncomeLedger) {
        toast.error('"Other income" ledger not found. Please create it first.');
    ...
    // For Add Charge: Debit is Farmer ledger, Credit is Other income ledger
    const payload = {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent className="max-w-md">
        ...
        ) : !otherIncomeLedger || !farmerLedger ? (
          <div className="py-4">
            <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
              <p className="text-sm text-yellow-800">
                {!otherIncomeLedger && (
                  <span className="block mb-2">⚠️ "Other income" ledger not found. Please create it first.</span>
                )}
                {!farmerLedger && (
