...
interface SellPotatoFormProps {
  isOpen: boolean;
  onClose: () => void;
  farmer: Farmer | null;
}

const SellPotatoForm = ({ isOpen, onClose, farmer }: SellPotatoFormProps) => {
  ...
  const potatoSalesLedger = ledgers.find((ledger) => 
    ledger.name.toLowerCase() === 'potato sales'
  );
  ...
  const handleSubmit = () => {
    if (!formData.amount || !potatoSalesLedger || !farmerLedger) {
      if (!potatoSalesLedger) {
        toast.error('Potato Sales ledger not found. Please create it first.');
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle>Sell Potato</DialogTitle>
          <DialogDescription>
            Create a voucher for potato sale to {farmer?.name}
          </DialogDescription>
        </DialogHeader>
        ...
        ) : !potatoSalesLedger || !farmerLedger ? (
          <div className="py-4">
            <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
              <p className="text-sm text-yellow-800">
                {!potatoSalesLedger && (
                  <span className="block mb-2">⚠️ "Potato Sales" ledger not found. Please create it first.</span>
                )}
                {!farmerLedger && (
