...
interface BuyPotatoFormProps {
  isOpen: boolean;
  onClose: () => void;
  farmer: Farmer | null;
}

const BuyPotatoForm = ({ isOpen, onClose, farmer }: BuyPotatoFormProps) => {
  ...
  const potatoPurchaseLedger = ledgers.find((ledger) => 
    ledger.name.toLowerCase() === 'potato purchase'
  );
  ...
  const handleSubmit = () => {
    if (!formData.amount || !potatoPurchaseLedger || !farmerLedger) {
      if (!potatoPurchaseLedger) {
        toast.error('Potato Purchase ledger not found. Please create it first.');
    ...
    const payload = {
      ...
      debitLedger: potatoPurchaseLedger._id,
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle>Buy Potato</DialogTitle>
          <DialogDescription>
            Create a voucher for potato purchase from {farmer?.name}
          </DialogDescription>
        </DialogHeader>
        ...
        ) : !potatoPurchaseLedger || !farmerLedger ? (
          <div className="py-4">
            <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
              <p className="text-sm text-yellow-800">
                {!potatoPurchaseLedger && (
                  <span className="block mb-2">⚠️ "Potato Purchase" ledger not found. Please create it first.</span>
                )}
                {!farmerLedger && (
