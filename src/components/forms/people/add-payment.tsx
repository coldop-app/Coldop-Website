...
const AddPaymentForm = memo(function AddPaymentForm({
  ...
  const form = useForm({
    defaultValues: {
      ...
      creditLedger: '' as string,
      amount: 0,
      narration: '',
    },
    onSubmit: async ({ value }) => {
      if (!farmerLedger || !value.creditLedger) return;
      const payload: CreateVoucherBody = {
        date: formatDateToISO(value.date),
        debitLedger: farmerLedger._id,
        creditLedger: value.creditLedger,
        amount: value.amount,
  ...
  return (
    ...
    >
      {/* Payment method (credit) dropdown */}
          ...
          children={(field) => {
            ...
            return (
              <Field data-invalid={isInvalid} className="gap-1.5 sm:gap-3">
                <FieldLabel htmlFor="add-payment-method">
                  Payment method (Credit ledger){' '}
      ...
      {/* Fixed ledger display: Debit = farmer, Credit = selected method (subscribed so card updates when dropdown changes) */}
      <form.Subscribe
        selector={(state: { values: { creditLedger: string } }) =>
          state.values.creditLedger
        }
      >
        {(creditLedgerId: string) => {
          const selectedCreditLedger = paymentMethodLedgers.find(
            (l) => l._id === creditLedgerId
          );
          return (
      ...
      <FieldGroup className="space-y-2 sm:space-y-4">
        ...
        <form.Field
          ...
          children={(field) => {
            ...
            return (
              <Field data-invalid={isInvalid} className="gap-1.5 sm:gap-3">
                <FieldLabel htmlFor="add-payment-amount">
        ...
        <form.Field
          ...
          children={(field) => (
            <Field className="gap-1.5 sm:gap-3">
              <FieldLabel htmlFor="add-payment-narration">Remarks</FieldLabel>
              <Input
                id="add-payment-narration"
                name={field.name}
        ...
        {(creditLedgerId: string) => (
          <div className="flex flex-col-reverse gap-2 pt-0.5 sm:flex-row sm:justify-end sm:gap-3 sm:pt-2">
            ...
            <Button
              type="submit"
              className="font-custom focus-visible:ring-primary h-9 min-h-9 w-full focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 sm:h-9 sm:min-h-0 sm:w-auto"
              disabled={isPending || !canSubmit || !creditLedgerId}
            >
