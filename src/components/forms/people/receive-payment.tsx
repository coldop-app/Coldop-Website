...
const PAYMENT_METHOD_CATEGORIES = ['Cash', 'Bank Accounts'] as const;

function isPaymentMethodLedger(ledger: Ledger): boolean {
  return PAYMENT_METHOD_CATEGORIES.includes(
    ledger.category as (typeof PAYMENT_METHOD_CATEGORIES)[number]
  );
}
...
export interface ReceivePaymentFormProps {
  ...
  /** Farmer name for subtitle and credit ledger display */
  farmerName?: string;
}

const ReceivePaymentForm = memo(function ReceivePaymentForm({
  onSuccess,
  onCancel,
  ...
  const { paymentMethodLedgers, farmerLedger } = useMemo(() => {
    const list = ledgers ?? [];
    const payment = list.filter(isPaymentMethodLedger);
    const farmer = defaultFarmerStorageLinkId
      ? list.find((l) => l.farmerStorageLinkId === defaultFarmerStorageLinkId)
      : null;
    return {
      paymentMethodLedgers: payment,
      farmerLedger: farmer ?? null,
    };
  }, [ledgers, defaultFarmerStorageLinkId]);

  const paymentMethodOptions: Option<string>[] = useMemo(
    () =>
      paymentMethodLedgers.map((l) => ({
        value: l._id,
        label: l.name,
  ...
  const canSubmit = Boolean(farmerLedger);

  const form = useForm({
    defaultValues: {
      date: formatDate(new Date()),
      debitLedger: '' as string,
      amount: 0,
      narration: '',
    },
    onSubmit: async ({ value }) => {
      if (!farmerLedger || !value.debitLedger) return;
      const payload: CreateVoucherBody = {
        date: formatDateToISO(value.date),
        debitLedger: value.debitLedger,
  ...
  return (
    ...
    >
          ...
          children={(field) => {
            ...
            return (
              <Field data-invalid={isInvalid} className="gap-1.5 sm:gap-3">
                <FieldLabel htmlFor="receive-payment-method">
                  Payment method (Debit ledger){' '}
                ...
                <SearchSelector
                  ...
                  emptyMessage="No Cash or Bank Accounts ledgers found"
                />
                {isInvalid && (
                  <FieldError
                    errors={
      ...
      {/* Fixed ledger display: Debit = selected method, Credit = farmer (subscribed so card updates when dropdown changes) */}
      <form.Subscribe
        selector={(state: { values: { debitLedger: string } }) =>
          state.values.debitLedger
        }
      >
        {(debitLedgerId: string) => {
          const selectedDebitLedger = paymentMethodLedgers.find(
            (l) => l._id === debitLedgerId
          );
          return (
      ...
      <FieldGroup className="space-y-2 sm:space-y-4">
        ...
        <form.Field
          ...
          children={(field) => {
            ...
            return (
              <Field data-invalid={isInvalid} className="gap-1.5 sm:gap-3">
                <FieldLabel htmlFor="receive-payment-amount">
        ...
        <form.Field
          ...
          children={(field) => (
            <Field className="gap-1.5 sm:gap-3">
              <FieldLabel htmlFor="receive-payment-narration">
                Remarks
              </FieldLabel>
              <Input
                id="receive-payment-narration"
                name={field.name}
        ...
        {(debitLedgerId: string) => (
          <div className="flex flex-col-reverse gap-2 pt-0.5 sm:flex-row sm:justify-end sm:gap-3 sm:pt-2">
            ...
            <Button
              type="submit"
              className="font-custom focus-visible:ring-primary h-9 min-h-9 w-full focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 sm:h-9 sm:min-h-0 sm:w-auto"
              disabled={isPending || !canSubmit || !debitLedgerId}
            >
