...
const DISCOUNT_LEDGER_NAME = 'Discount';

export interface AddDiscountFormProps {
  onSuccess?: () => void;
  onCancel?: () => void;
...
const AddDiscountForm = memo(function AddDiscountForm({
  onSuccess,
  onCancel,
  ...
  const { discountLedger, farmerLedger } = useMemo(() => {
    const list = ledgers ?? [];
    const discount =
      list.find(
        (l) => l.name === DISCOUNT_LEDGER_NAME && (l.isSystemLedger ?? true)
      ) ?? list.find((l) => l.name === DISCOUNT_LEDGER_NAME);
    const farmer = defaultFarmerStorageLinkId
      ? list.find((l) => l.farmerStorageLinkId === defaultFarmerStorageLinkId)
      : null;
    return {
      discountLedger: discount ?? null,
      farmerLedger: farmer ?? null,
    };
  }, [ledgers, defaultFarmerStorageLinkId]);

  const canSubmit = Boolean(discountLedger) && Boolean(farmerLedger);
  ...
  const form = useForm({
    ...
    onSubmit: async ({ value }) => {
      if (!discountLedger || !farmerLedger) return;
    ...
    >
      ...
      <FieldGroup className="space-y-2 sm:space-y-4">
          ...
          children={(field) => {
            ...
            return (
              <Field data-invalid={isInvalid} className="gap-1.5 sm:gap-3">
                <FieldLabel htmlFor="add-discount-amount">
        ...
        <form.Field
          ...
          children={(field) => (
            <Field className="gap-1.5 sm:gap-3">
              <FieldLabel htmlFor="add-discount-narration">Remarks</FieldLabel>
              <Input
                id="add-discount-narration"
                name={field.name}
