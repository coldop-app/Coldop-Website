...
const ShedVoucherCard = ({ order }: ShedVoucherCardProps) => {
  ...
  const shareCard = (cardData: Order) => {
    const message: WebViewMessage = {
      type: 'SHARE_CARD',
      title: "Shed Voucher " + cardData.voucher.voucherNumber,
  ...
  const calculateLotNo = () => {
    return order.orderDetails.reduce((total, detail) =>
      total + detail.bagSizes.reduce((sum, bag: any) =>
        sum + ((bag as any).quantityRemoved || 0), 0
      ), 0
    );
  };

  // Calculate total cost (shed cost per bag × number of bags)
  const calculateTotalCost = () => {
    if (order.shedCost === undefined) return 0;
    const totalBags = calculateLotNo();
    return order.shedCost * totalBags;
  };
  ...
  const sortedOrderDetails = useMemo(() => {
    return order.orderDetails.map(detail => ({
      ...detail,
      bagSizes: sortBagSizes(detail.bagSizes.map((bag: any) => bag.size)).map(sortedSize =>
        detail.bagSizes.find((bag: any) => bag.size === sortedSize)!
      )
    }));
      ...
      if (isWebView()) {
        ...
        try {
          ...
          reader.onload = function() {
            ...
            const fileName = `Shed_Voucher_${order.voucher.voucherNumber}_${new Date().toISOString().split('T')[0]}.pdf`;

            const message: WebViewPDFMessage = {
              type: 'OPEN_PDF_NATIVE',
              title: `Shed Voucher ${order.voucher.voucherNumber}`,
              fileName: fileName,
              pdfData: base64Data
            };
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 mb-4">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-medium text-gray-900">
              Shed Voucher: <span className="text-yellow-600">{order.voucher.voucherNumber}</span>
            </span>
          </div>
          <div className="flex items-center gap-2">
            ...
            {order.shedCost !== undefined && (
              <div className="text-xs text-gray-600 bg-gray-50 px-2 py-1 rounded-md">
                Total Cost: <span className="font-medium text-gray-900">₹{calculateTotalCost()}</span>
              </div>
            )}
          </div>
        </div>
          ...
          <div className="flex-1">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-x-3 gap-y-3">
              <div className="min-w-0">
                ...
                <p className="text-sm font-medium text-gray-900 truncate">{order.orderDetails[0]?.variety || 'N/A'}</p>
              </div>
              <div className="min-w-0">
      ...
      {isExpanded && (
        <div className="border-t border-gray-100">
          <div className="p-3 sm:p-4 lg:p-5 space-y-6">
            ...
            {/* Net Shed Details */}
            <div className="bg-gray-50/50 rounded-xl p-4 border border-gray-100">
              <h3 className="text-sm font-medium text-gray-900 mb-4">Net Shed Details</h3>
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    <tr>
                      {sortBagSizes(Array.from(new Set(order.orderDetails.flatMap(d => d.bagSizes.map((b: any) => b.size))))).map((bagSize, idx) => {
                        const totalRemoved = order.orderDetails.reduce((total, detail) =>
                          total + detail.bagSizes.filter((bag: any) => bag.size === bagSize).reduce((sum, bag: any) =>
                            sum + ((bag as any).quantityRemoved || 0), 0
                          ), 0
                        );
                        ...
                        return (
                          <td key={idx} className="py-3 px-3 text-center">
                            <span className="font-medium text-yellow-600">{totalRemoved}</span>
                          </td>
                        );
                      })}
                      <td className="py-3 px-3 text-center">
                        <span className="font-medium text-yellow-600">{calculateLotNo()}</span>
                      </td>
                    </tr>
                  </tbody>
                </table>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="bg-gray-50">
                      ...
                      <th className="text-right py-3 px-4 font-medium text-gray-900">Qty Moved</th>
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {sortedOrderDetails.map((detail, detailIndex) => (
                      detail.bagSizes.map((bagSize: any, bagIndex) => {
                        const incomingBagSize = (detail.incomingOrder as any)?.incomingBagSizes?.find(
                          (b: any) => b.size === bagSize.size
                        );
                        const currentQuantity = incomingBagSize?.currentQuantity || 0;
                        const removedQuantity = (bagSize as any).quantityRemoved || 0;
                        ...
                        return (
                          <tr key={`${detailIndex}-${bagIndex}`} className="hover:bg-gray-50/50 transition-colors">
                            ...
                            <td className="py-3 px-4 text-gray-700">
                              {(bagSize as any).location || 'N/A'}
                            </td>
                            <td className="py-3 px-4">
                              {(detail.incomingOrder as any) ? (
                                <div className="flex items-center gap-2">
                                  <div className="w-1.5 h-1.5 bg-green-500 rounded-full"></div>
                                  <span className="text-sm font-medium text-gray-900">
                                    {(detail.incomingOrder as any).voucher.voucherNumber}
                                  </span>
                                </div>
                              ) : (
              ...
              <div className="bg-yellow-50/50 rounded-xl p-4 border border-yellow-100">
                <div className="flex justify-between items-center mb-2">
                  ...
                  <div className="text-xs text-gray-500">
                    (₹{order.shedCost} × {calculateLotNo()} bags)
                  </div>
                </div>
                <p className="text-lg font-semibold text-yellow-600">₹{calculateTotalCost()}</p>
              </div>
            )}
