...
const PeopleScreen = () => {
  ...
  const [sortBy, setSortBy] = useState<"name" | "createdAt" | "farmerId">(
    "farmerId"
  );
      ...
      if (error instanceof Error) {
        ...
        toast.error(
          apiError.response?.data?.message ||
            t("people.errors.failedToCreateFarmer")
        );
      } else {
              ...
              <div className="w-full sm:w-[200px]">
                ...
                {sortOpen && (
                  <div className="absolute z-10 mt-1 w-[200px] bg-white border border-gray-200 rounded-lg shadow-lg">
                    <button
                      className={`block w-full text-left px-4 py-2.5 text-sm hover:bg-gray-50/50 transition-colors ${
                        sortBy === "name"
          ...
          {farmers.length === 0 ? (
            ...
            farmers.map((farmer) => (
              <div
                ...
                onClick={() =>
                  navigate(`/erp/people/${farmer._id}`, {
                    state: {
                      farmer: {
                        _id: farmer._id,
                        name: farmer.name,
