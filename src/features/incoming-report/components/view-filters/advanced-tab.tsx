...
function getColumnLabel(column: AppColumn<IncomingGatePassReportRecord>) {
  return column.columnDef.meta?.filterLabel ?? column.id;
}

function getDefaultOperator(column: AppColumn<IncomingGatePassReportRecord>) {
  return isAdvancedNumericColumn(column.id) ? 'greaterThan' : 'contains';
}

function getOperatorOptions(column: AppColumn<IncomingGatePassReportRecord> | undefined) {
  return column && isAdvancedNumericColumn(column.id)
    ? NUMERIC_OPERATOR_OPTIONS
    : STRING_OPERATOR_OPTIONS;
}

function getColumnValueOptions(column: AppColumn<IncomingGatePassReportRecord> | undefined) {
  if (!column) return [];
...
function createCondition(column: AppColumn<IncomingGatePassReportRecord>): AdvancedFilterCondition {
  return {
