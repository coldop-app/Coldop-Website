...
function getOperatorOptions(column: Column<IncomingGatePassReportRecord, unknown> | undefined) {
  return column && isAdvancedNumericColumn(column.id)
    ? NUMERIC_OPERATOR_OPTIONS
    : STRING_OPERATOR_OPTIONS;
}

function getColumnValueOptions(column: Column<IncomingGatePassReportRecord, unknown> | undefined) {
  if (!column) return [];
