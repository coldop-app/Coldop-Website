  ...
  return (
    <Card size="sm" className="card-hover gap-0">
      <CardHeader className="pb-2">
        ...
        <CardAction>
          <div className="bg-primary/10 group-hover/card:bg-primary/15 flex size-9 items-center justify-center rounded-xl transition-colors duration-200">
            <Icon
...
export function ChamberAnalyticsSummaryCards({ cards }: ChamberAnalyticsSummaryCardsProps) {
  return (
    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
      {CHAMBER_SUMMARY_CARD_CONFIG.map(({ key, icon }) => {
        const card = cards[key];
        const sentinel = key === 'topChamber' && isSentinelLabel(card.value);
