...
export function buildOutgoingReportPreviewData({
  ...
  const bodyRows: ExcelPreviewRow[] = exportRows.map((row) => {
    ...
    const values = visibleColumns.map((column) => {
      const exportCell = getExportCellForRow(row, column, quantityMode, showLocation);
  ...
  return {
    ...
    filterSummaryLines,
    fileName: sanitizeFilename(`outgoing-report_${format(generatedAt, 'yyyy-MM-dd')}.xlsx`),
  };
}
